...
we have effective tools for cross-referencing searches. Our software is contained in the `gwasCatSearch` package (@citegwasCatSearch) 
...
We next add a column to the metadata for the `efo_tc` corpus. This then allows us to subset the corpus and then we can carry out analyses based on the subset. Details on these capabilities are given in the @citecorpustools package vignette.
...
FIXME: Explain how to look at genes, or manhattan plot etc
describe use of granges here

Once we have a set of GWAS studies that we are interested in we can access the metadata
about which variants, where they are in the genome, etc.  This information is quite
large and we use a SQLite database as the main tool for storing and accessing the data.

These data are accessed via either `granges_from_study` if you want a `granges` object that
can be integrated with a variety of Bioconductor @citeBioc tools for analyzing and plotting genomic data, or `variants_from_study` which returns a dataframe with 21 columns providing a set of
metadata.

719 ranges and 21 metadata columns:
...
6,700 studies is titled: Identification of 969 protein quantitative trait loci in an African American population with kidney disease attributed to hypertension @citeckdPaper.
