...
The disease location information was extracted directly from EFO statements of the form:  X has_disease_location Y, where Y is typically an UBERON (@Uberon2012) term representing an anatomical location. If a term does not have an explicitly stated location relationship, we determine if it has an inferred location. This is done by recursively checking if a parent in the ontology hierarchy has a location. The first location found is used, and if none is found then it is marked as missing. 
...
knitr::kable(manystudies[,c(1,2,5,6)], row.names=FALSE, caption="List of EFO terms with more than 500 Studies", label="tab:xx")
...
##FIXME- the matches are largely in the measurement area
