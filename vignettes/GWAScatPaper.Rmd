...
research interest in large data resources such as NHANES or biobanks can be aided by effective use of
...
*Each study is also assigned a trait that best represents the phenotype under investigation. When multiple traits are analysed in the same study either multiple entries are created, or individual SNPs are annotated with their specific traits.*. In some situations individual SNPs will be annotated with a trait, while in other cases collections of SNPs will be associated with a single trait.  This occurs, for example, when publications report SNP x SNP interactions.
