...
knitr::kable(t(GWCexp1), align='lc', caption="Study GCST90083943", col.names="") |> kable_styling()
...
For this search we start fairly broadly and search the GWAS catalog for the term *infectious*. One could also search for specific diseases or other terms and then merge the results.
This search uses the GWAS  the text descriptions of the GWAS trait as well as the trait and synonyms that correspond to the EFO term(s) this study was directly mapped to.
...
Once we have a set of GWAS studies that we are interested in we can access the metadata about which variants are implicated, where they are in the genome, the p-value and effect size to name a few of the attributes that are provided.  These data are large and we use a  database as the main tool for storing and accessing the data. It is straightforward to read the data store them in standard Bioconductor objects and then employ the extensive array of tools for analyzing and plotting the data.
...
sort(table(tab$MAPPED_GENE),dec=TRUE)[1:10]
