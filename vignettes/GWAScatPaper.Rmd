...
Realizing the potential for discovery is hampered by many challenges including the complexity of
the data and the inherent challenge of phenotypes and experimental factors
being described primarily in natural language. We explore the challenges and opportunites that arise from mapping textual descriptions of traits and experimental conditions to ontologies. In particular we examine the use of corpus search tools (as exemplified in the `corpustools` package
...
@maloneModelingSampleVariables2010) we can also examine how the structure that mapping provides can be used to aid the analysis and interpretation of the data.

The GWAS Catalog (@sollisNHGRIEBIGWASCatalog2023) reports on a set of published curated GWAS
...
As part of the processing we construct two dataframes, one based on the EFO terms `efo_df` and the other based on the GWAS catalog studies `gwc_df`. These are described in Tables 1 and 2. 
...
kable_styling(kbl(ttz, caption="Overview of gwc_df"), bootstrap_options = "striped", full_width = FALSE)
```
...
```{r tryfig, echo=FALSE, message=FALSE, fig.cap="Terms and relations in EFO"}
...
We create two different corpora for searching. One based on the EFO and
...
The journal Genetics allows for statisical analysis or GWAS/TWAS etc analysis...
