...
```{r}
hits = search_features(efo_tc, query = c('Granuloma# granulo*', 'Rheumatic# rheum*'), context_level='sentence')
summary(hits)
hits$hits[1:10,]
table(hits$hits$sentence)
```

We can see that there were `r nrow(hits)` hits.
We see that most of the hits are in sentences 1, 2 or 3. But what we are most interested in might be which field the 
For a hit in sentence 1 we expect it then to be in the name of the trait assigned to the specific EFO term.  We show how to examine those in the next code chunk.  These are going to be the values in the `Object` column of `efo_df`.

```{r CheckHitsTrait}
## obtain the EFO ids for the hits in sentence 1
testID = hits$hits$doc_id[hits$hits$sentence==1]
length(testID)
head(efo_df[testID, "Object"])

```

There are two other ways that our query could have matched, one is via the synonyms that have been provided by the ontology and the second is that the query matches to the name of some other trait that has mapped directly to that term.
First we will look at synonym matching
```{r SynonymMatch}
synID = hits$hits$doc_id[hits$hits$sentence==2]
length(synID)

```
Similary we can look at the other matches, those are the names used for other traits in the GWAS catalog that map directly to the same ontological label.  These should all be in 
```{r}
hitsasDT = hits2DT(hits, efo_df, efo_tc)
datatable(hitsasDT, escape=FALSE, rownames=FALSE)
```

In the next code chunk we search for any EFO labels that match to both *granulo\** and *rheum\**.
...
In the next code segment we show how to get the synonyms and the text labels for any phenotypes that have been directly mapped to a specific EFO term. We consider the term EFO:0000095 which is labeled: chronic lymphocytic leukemia.

```{r showSynonymDirectMapping}
syns = getSynonyms("EFO:0000095")
length(syns$"EFO:0000095")
syns$`EFO:0000095`[1:10]

```

We first show 10 of the `r length(syns$"EFO:0000095")` synonyms provided by the EFO ontology.
