...
We see that most of the hits are in the Matches and the Synonyms, which makes some sense, as these fields will tend to have multiple entries, so there is more text there to match. 
...
To obtain the synonyms for any ontology term you can use the getSynonyms function. Notice that some of the synonyms are actually acronyms. This allows you to find features from acronyms.
```{r GetSynonyms}
  testID[1]
